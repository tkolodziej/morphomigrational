...
function [sMMangle, MArotation, ResultsAuxiliary] = SignedMM(File, Settings, k, ResultsMain, ResultsAuxiliary, NameAppendix, FrameList);
...
%Values of sMM angle and MA orientation in previous frame 
        ...
        OrientationPrevMA = ResultsAuxiliary(k-1, 12);
   ...
   elseif ResultsAuxiliary(1, 11) < 0
       sMMangle = -sMMTemp;
   elseif ResultsAuxiliary(1,11) > 0 
       sMMangle = sMMTemp;
   end
...
% The major axis is rotated clockwise to the nearest horizontal position.
...
%Values of sMM angle and MA orientation in previous frame 
   ...
   % Now it's drawing time! Let's draw mask, with axes before and after 
   % rotation!
   ...
   %-These ifs decides about where the legend should be.
   ImageCenter = round(size(Mask)/2);
   if Sx1 < ImageCenter(2) && Sy1 < ImageCenter(1);
      LegendPlacement = 'southeast';
   elseif Sx1 > ImageCenter(2) && Sy1 < ImageCenter(1);
      LegendPlacement = 'southwest';
   elseif (Sx1 > ImageCenter(2)) && Sy1 > ImageCenter(1);
      LegendPlacement = 'northwest';
   elseif Sx1 < ImageCenter(2) && Sy1 > ImageCenter(1);
   ...
   sMMangleText=sprintf(['NaN (last frame)']);
   MArotationText = sprintf('%.2f',MArotation);
   FrameNumber = num2str(ResultsMain(k, 2))
  ...
  % Wybór zapisywania obrazka:
   ...
   hold on
        title(['Frame ', FrameNumber, '. ', 'sMM = ', sMMangleText,  '. ', 'Rotation of ', MArotationText, char(176)], 'FontSize', 10);
        xlabel([SequenceName], 'Interpreter', 'none')

        v1=plot([Ax Bx], [Ay By], '-b', 'LineWidth', 2,'DisplayName', 'MA');
        v3=plot([Px MAendAfterRotation(1)], [Py MAendAfterRotation(2)], '-m', 'LineWidth', 2, 'DisplayName', 'Rotated MA');   

        l=legend({'MA', 'Rotated MA'}, 'FontSize',5, 'Location', LegendPlacement);
