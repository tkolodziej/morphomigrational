...
Settings.Sampling = 4; % How densely the sequence should be sampled (1 - all frames, 2 - each second frame, 3 - each third frame, etc.)
Settings.TimeIntervalSec=150;% Time interval of the original time-lapse experiment IN SECONDS (regardless the chosen sampling)
...
File = OpenFiles(File, Settings); %Opens the binary masks.

File = CalculationsMajorScript(File, Settings); %Calculates all parameters
